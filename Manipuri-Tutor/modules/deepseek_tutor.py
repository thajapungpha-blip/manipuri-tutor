"""DeepSeek translator — English -> Manipuri (Bengali script).

Drop-in replacement for gemini_tutor: same public functions
(`translate_sentences`, `detect_poem`, `explain_full_document`) so app.py
only needs its import line changed.

DeepSeek's API is OpenAI-compatible, so we use the `openai` SDK pointed at
DeepSeek's base URL. Set DEEPSEEK_API_KEY in Streamlit secrets.
"""
...
# deepseek-chat = DeepSeek-V3 (text). Change here if you use another model.
_MODEL_NAME = "deepseek-chat"
_BASE_URL = "https://api.deepseek.com"
_BATCH_SIZE = 20
_INTER_CALL_DELAY_S = 1.0   # DeepSeek is pay-per-use, no tight free cap
...
def _parse_json(raw: str) -> dict:
    ...
    if match:
        try:
            ...
            pass
    raise RuntimeError(f"Could not parse DeepSeek JSON. First 200 chars: {raw[:200]}")
...
def _translate_batch(batch: list[str], is_poem: bool = False) -> list[dict]:
    ...
    if not isinstance(translations, list):
        raise RuntimeError("DeepSeek response missing 'translations' list.")
...
def translate_sentences(sentences: list[str], progress_cb=None,
    ...
    for bi, batch in enumerate(batches):
        ...
        def _on_wait(secs, attempt):
            if progress_cb:
                progress_cb(done, total,
                            f"Busy — waiting {int(secs)}s (retry {attempt})…")
